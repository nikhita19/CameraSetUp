...
class VanishingPoints:
  ...
  def third_vp(self, vp1, vp2, image):
    ...
    v = vp2.copy()
    pp = [0,0,0]
    pp[0] = image.shape[1]/2
    pp[1] = image.shape[0]/2
    prod = np.dot(u[0:2] - pp[0:2], v[0:2] - pp[0:2])
    f = math.sqrt(-prod)
    u[2] = f
    v[2] = f
    w = np.cross(u - pp, v - pp)
    vp3 = np.concatenate((w[0:2] / w[2] * f + pp[0:2], [1]))
    vp3Direction = np.concatenate((vp3[0:2], [f])) - pp
    roadPlane = np.concatenate((vp3Direction / np.linalg.norm(vp3Direction), [10]))
    self.vis_model(image, vp3)
    return vp3, f, pp, roadPlane
  ...
  def all_vp(self):
    ...
    vp3, focal, pp, roadPlane = self.third_vp(vp1,vp2,image)
    return vp1, vp2, vp3, focal, pp, roadPlane